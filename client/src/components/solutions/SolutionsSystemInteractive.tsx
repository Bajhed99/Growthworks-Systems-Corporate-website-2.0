...
const cards = [
  {
    ...
    detail:
      "A weak digital presence means qualified buyers are finding competitors instead. We build the foundation that earns discovery â€” optimized positioning, authoritative content, and a website that converts the traffic it earns.",
    stat: "3Ã—",
  ...
  },
  {
    ...
    detail:
      "Most lost deals are not lost to competition â€” they are lost to silence. We implement pipeline discipline, coaching checkpoints, and CRM hygiene so your team closes predictably regardless of rep experience or deal complexity.",
    stat: "41%",
  ...
  },
  {
    ...
    detail:
      "Revenue Intelligence connects every stage into a single feedback loop. You see where leads stall, which messages convert, and which reps need support â€” then that knowledge feeds back upstream to sharpen everything before it.",
    stat: "â†‘",
    ...
    <div className="size-full bg-white overflow-auto" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <section style={{ backgroundColor: "#ffffff" }}>
        ...
        <div style={{ background: "#f5f0e8", borderBottom: "1px solid #e0d9cf", paddingBlock: 14, textAlign: "center" }}>
          <span style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: "0.18em", color: "var(--maroon)", textTransform: "uppercase" }}>
            The Solution System
          </span>
        </div>
        ...
        <div style={{ maxWidth: 860, marginInline: "auto", paddingBlock: "72px 64px", paddingInline: 24, textAlign: "center" }}>
          ...
          <p style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 18, fontWeight: 400, lineHeight: 1.7, color: "#4a4744", maxWidth: 600, marginInline: "auto" }}>
            Four solution areas organized around the business responsibility they strengthen â€” not individual tools or technologies. Connected by design.
          </p>
        </div>
              ...
              <div key={card.num} style={{ display: "flex", alignItems: "center", flex: i < 3 ? 1 : "none" }}>
                ...
                >
                  <div style={{
                    ...
                    height: 36,
                    borderRadius: "50%",
                    border: active === i ? "2px solid var(--maroon)" : "1px solid #d4cfc8",
                    background: active === i ? "var(--maroon)" : "#ffffff",
                    display: "flex",
                    alignItems: "center",
                    ...
                    <span style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 10, fontWeight: 600, color: active === i ? "#ffffff" : "var(--maroon)", letterSpacing: "0.06em", position: "relative", zIndex: 1 }}>
                      {card.num}
                    </span>
                  </div>
                  <span style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 11, fontWeight: active === i ? 600 : 400, color: active === i ? "var(--maroon)" : "#9b9590", letterSpacing: "0.04em", whiteSpace: "nowrap", transition: "all 0.3s ease" }}>
                    {card.title}
                  </span>
                </button>
                {i < 3 && (
        ...
        <div style={{ maxWidth: 1200, marginInline: "auto", paddingInline: 24, marginBottom: 0 }}>
          <div
            ...
            style={{
              ...
              border: `1.5px solid ${c.accent ? "var(--maroon)" : "#d4cfc8"}`,
              background: c.accent ? "var(--maroon)" : "#ffffff",
          ...
          >
            ...
            <div style={{ padding: "52px 48px", borderRight: `1px solid ${c.accent ? "rgba(255,255,255,0.15)" : "#d4cfc8"}`, position: "relative", overflow: "hidden" }}>
              ...
              <p style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: c.accent ? "rgba(255,255,255,0.5)" : "var(--maroon)", marginBottom: 20 }}>
                {c.tag}
              </p>
              ...
              <div style={{ width: 36, height: 2, background: c.accent ? "rgba(255,255,255,0.3)" : "var(--maroon)", marginBottom: 24 }} />
            ...
            <div style={{ padding: "52px 48px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              ...
              <div style={{ marginTop: 40, paddingTop: 28, borderTop: `1px solid ${c.accent ? "rgba(255,255,255,0.15)" : "#e0d9cf"}`, display: "flex", alignItems: "baseline", gap: 14 }}>
                <span style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: 48, fontWeight: 400, color: c.accent ? "#ffffff" : "var(--maroon)", lineHeight: 1 }}>
                  {c.stat}
                </span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0, borderLeft: "1px solid #d4cfc8", borderRight: "1px solid #d4cfc8", borderBottom: "1px solid #d4cfc8" }}>
            {cards.map((card, i) => (
              <button
                ...
                style={{
                  textAlign: "left",
                  background: active === i ? (card.accent ? "#8a1717" : "#faf8f4") : (card.accent ? "var(--maroon)" : "#ffffff"),
                  borderRight: i < 3 ? "1px solid #d4cfc8" : "none",
                  borderTop: `2px solid ${active === i ? "var(--maroon)" : "transparent"}`,
                  padding: "24px 28px 20px",
                  cursor: "pointer",
                  transition: "background 0.25s ease",
                  border: "none",
                  borderRight: i < 3 ? "1px solid #d4cfc8" : "none",
                  borderTop: active === i ? "2px solid var(--maroon)" : "2px solid transparent",
                  position: "relative",
                  overflow: "hidden",
                }}
              >
                <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: active === i ? (card.accent ? "rgba(255,255,255,0.6)" : "var(--maroon)") : (card.accent ? "rgba(255,255,255,0.5)" : "#c0b8b0"), marginBottom: 6 }}>
                  {card.num}
                </div>
                ...
                <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 11, color: active === i ? (card.accent ? "rgba(255,255,255,0.5)" : "var(--maroon)") : (card.accent ? "rgba(255,255,255,0.4)" : "#b0a8a0"), marginTop: 4 }}>
                  {card.tag}
                </div>
                {/* Progress sweep */}
                {active === i && (
                  <div style={{ position: "absolute", bottom: 0, left: 0, height: 2, background: card.accent ? "rgba(255,255,255,0.4)" : "var(--maroon)", width: `${progress * 100}%`, transition: "width 0.05s linear" }} />
                )}
              </button>
            ))}
          </div>
        ...
        <div style={{ textAlign: "center", padding: "20px 24px", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--maroon)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{ opacity: 0.45 }}>
        ...
        <div style={{ borderTop: "1px solid #e0d9cf", background: "#f5f0e8", paddingBlock: 52, paddingInline: 24, textAlign: "center" }}>
          <p style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 18, fontWeight: 400, lineHeight: 1.7, color: "#4a4744", maxWidth: 580, marginInline: "auto" }}>
            ...
            <span style={{ color: "var(--maroon)", fontWeight: 500 }}>Learn how they work together below.</span>
          </p>
        </div>
